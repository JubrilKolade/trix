...
describe('Generators', () => {
  test('generateConfigFile writes trix.config.json', async () => {
    ...
    const config = { projectType: 'frontend', projectName: 'test-app' };
  ...
  test('generatePackageJson returns package json object', async () => {
    const config = { projectType: 'frontend', projectName: 'pkg-test', framework: 'react', styling: 'vanilla', uiComponents: [], apiClient: 'fetch', stateManagement: 'none' } as any;
